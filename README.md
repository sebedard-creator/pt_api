...
- **Verified root and production-virtual clips:** Native Pro Tools root and virtual layouts using flags `0x0001`, `0x2001`, `0x3001` or `0x4001` are supported by `relink_clip()` when their observed native media/catalog layouts are present. The media reference is `placement_start_samples - src_offset`; the placement must not precede its source offset.
