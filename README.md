...
| | `get_relink_write_status(track_name, clip_name, placement_start_samples)` | Read-only preflight for a specific placement. It verifies the exact supported `0x2628` geometry and media header before a caller creates a WAV or calls `relink_clip()`. A positive result does not waive the remaining catalog, WAV, name and placement checks performed by the writer. |
...
- **Verified root, parent and virtual production clips:** Native Pro Tools root and production layouts using flags `0x0000`, `0x0001`, `0x2000`, `0x2001`, `0x3000`, `0x3001` or `0x4001` are supported by `relink_clip()` when their observed native media/catalog layouts are present. This includes the Pro Tools/RX-rendered `0x3000 / 0x20 / 0x44 / 0x08` geometry (UInt24 source offset, UInt16 length). The original flag, source-offset width and embedded timing geometry are preserved; the new media reference is `placement_start_samples - src_offset`, so the placement must not precede its source offset.
...
- **Safe caller preflight:** `get_relink_write_status()` is read-only and resolves one exact placement before validating its `0x2628` geometry and inspecting its virtual-media header. It returns `supported=False`, code `unsupported_clip_layout` for a decoded but unwritable geometry, `unverified_clip_layout` for an incomplete geometry, and `premiere_virtual_media` with `detail_header_length` for the known variable-header case (for example 173 bytes). It also returns a conservative `unverified_*` status if the media catalog/header cannot be verified. A positive result does not waive catalog, WAV, name and placement validation, but it never represents a geometry the writer itself rejects.
