...
- La suite automatisée compte 194 tests et passe intégralement. Elle inclut les préflights de relink parent/virtuel, la variante RX `0x3000/0x20/0x44/0x08`, le rejet sans mutation d'un header Premiere `0x2106` de 173 octets, la lecture des occurrences de Clip Groups et les deux profils du builder.
...
- métadonnées PEP 517 alignées sur `1.4.2`; la construction du wheel `pt_api-1.4.2-py3-none-any.whl` reste à exécuter dans l'environnement de release équipé de `build`/`wheel` (ces outils ne sont pas installés sur cette station);
